...
public final class LongSaturatedMathKt {
    ...
    public static final long m1944saturatingAddNuflL3o(long j, DurationUnit unit, long j2) {
        Intrinsics.checkNotNullParameter(unit, "unit");
        long m1861toLongimpl = Duration.m1861toLongimpl(j2, unit);
        if (((j - 1) | 1) == Long.MAX_VALUE) {
            return m1943checkInfiniteSumDefinedPjuGub4(j, j2, m1861toLongimpl);
        }
        if ((1 | (m1861toLongimpl - 1)) == Long.MAX_VALUE) {
            return m1945saturatingAddInHalvesNuflL3o(j, unit, j2);
        }
        long j3 = j + m1861toLongimpl;
        return ((j ^ j3) & (m1861toLongimpl ^ j3)) < 0 ? j < 0 ? Long.MIN_VALUE : Long.MAX_VALUE : j3;
    }

    /* renamed from: checkInfiniteSumDefined-PjuGub4, reason: not valid java name */
    private static final long m1943checkInfiniteSumDefinedPjuGub4(long j, long j2, long j3) {
        if (!Duration.m1847isInfiniteimpl(j2) || (j ^ j3) >= 0) {
            return j;
        }
    ...
    private static final long m1945saturatingAddInHalvesNuflL3o(long j, DurationUnit durationUnit, long j2) {
        long m1818divUwyO8pc = Duration.m1818divUwyO8pc(j2, 2);
        long m1861toLongimpl = Duration.m1861toLongimpl(m1818divUwyO8pc, durationUnit);
        return (1 | (m1861toLongimpl - 1)) == Long.MAX_VALUE ? m1861toLongimpl : m1944saturatingAddNuflL3o(m1944saturatingAddNuflL3o(j, durationUnit, m1818divUwyO8pc), durationUnit, Duration.m1850minusLRDsOJo(j2, m1818divUwyO8pc));
    }

    private static final long infinityOfSign(long j) {
        return j < 0 ? Duration.INSTANCE.m1917getNEG_INFINITEUwyO8pc$kotlin_stdlib() : Duration.INSTANCE.m1916getINFINITEUwyO8pc();
    }

    public static final long saturatingDiff(long j, long j2, DurationUnit unit) {
        Intrinsics.checkNotNullParameter(unit, "unit");
        if ((1 | (j2 - 1)) == Long.MAX_VALUE) {
            return Duration.m1868unaryMinusUwyO8pc(infinityOfSign(j2));
        }
    ...
    private static final long saturatingFiniteDiff(long j, long j2, DurationUnit durationUnit) {
        ...
        if (((j3 ^ j) & (~(j3 ^ j2))) < 0) {
            if (durationUnit.compareTo(DurationUnit.MILLISECONDS) < 0) {
                ...
                return Duration.m1851plusLRDsOJo(DurationKt.toDuration(j4, DurationUnit.MILLISECONDS), DurationKt.toDuration(j5, durationUnit));
            }
            return Duration.m1868unaryMinusUwyO8pc(infinityOfSign(j3));
        }
